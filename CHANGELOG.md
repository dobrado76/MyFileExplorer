...
- **Drive free space** — opening a drive shows `N GB free of M GB (P%)` in the status bar. Click the tree **Drives** header for every volume (pies in the preview; offline letters as Disconnected). Space is queried only for local disks, with a short timeout so one dead letter cannot hide the rest.
