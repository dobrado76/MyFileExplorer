...
- **Image Edit crop handles** — the editor keeps Crop/theme/callback props stable so a background re-render does not rebuild Filerobot config mid-drag (which snapped handles back to the gesture start).
...
- **Image Edit multi-op session** — rotate then crop (and other Adjust tools) stays in memory without Save between steps. Entering Crop after rotate/flip bakes the canvas into a new in-memory source so Filerobot’s Crop tool cannot snap back to the unrotated original.
...
- **Icon-view thumbnails no longer stall after scrolling** — fast-scrolling Extra large / Large / Medium / Small icons queued unbounded Sharp encodes on main, which also blocked preview. `thumbs:get` is throttled (newest visible tiles first; off-screen waiters dropped) and Sharp generate is capped; already-cached thumbs still skip the encode queue.
...
- **Rich player (mpv)** — Settings → Preview → **Rich player (mpv)** (off by default). When on, Preview places an **owned borderless mpv overlay** for containers without a Chromium `mediaUrl` (MKV/WMV/AVI/…). Fetch **pinned** binaries with `npm run tools:fetch-mpv` (`scripts/mpv-pin.json` + SHA-256; notices in `tools/mpv/THIRD_PARTY_NOTICES.md`). Still no ffmpeg remux. No Chromium→mpv fallthrough.
- **Rich player overlay** — spawn mpv (not `--wid` / not `WS_CHILD` under Electron GPU), find via `--title` + `FindWindowW`, park as owned top-level overlay. Session generation token avoids orphan processes on rapid nav; bounds/stop IPC are owner-scoped.
...
- **Shell redirect restore safety (D72)** — uninstall restores via install-dir or userData sidecar launcher and Aborts if redirect cannot be restored; restore never deletes `open`/`explore` without a valid backup; delete-then-import exact restore; backup cleared after verified restore; `DelegateExecute` cleared on enable and required absent for “Enabled”; repair is transactional (full live subtree snapshot/rollback, not command-only); launcher logs failed spawns as `error` and trims the invocation log. See [docs/WINDOWS_SHELL_REDIRECT.md](docs/WINDOWS_SHELL_REDIRECT.md).
...
- **Virtual Folder OS projection (D68, Windows)** — optional WinFsp in-place sibling mount (`Name.mfevirtual` → `Name\`) via independent .NET service; Settings → Behavior (win32). GitHub Releases include `MfeVirtualFolderService-win-x64.zip`; install [WinFsp](https://winfsp.dev/) separately. See [docs/VIRTUAL_FOLDER_PROJECTION.md](docs/VIRTUAL_FOLDER_PROJECTION.md).
- **Virtual Folders (`.mfevirtual`)** — portable JSON folder-like collections of path references (D67). Open as a first-class folder location (document path, not fake nested paths); New → Virtual Folder; drop/paste adds refs; Del removes membership; extract group ↔ absorb document; Manual sort + Location column; dedicated preview. See [docs/VIRTUAL_FOLDERS.md](docs/VIRTUAL_FOLDERS.md).
...
- **Virtual Folder OS projection** — shipped for all Windows users (not DEV-gated); requires WinFsp + the projection service and Settings → Behavior → Virtual Folder OS projection.
...
- **`PLAN.md`** — planning doc retired; product intent lives in `docs/` (especially DECISIONS / PRODUCT_SPEC).
