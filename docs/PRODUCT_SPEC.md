...
- Sort by any visible column (incl. media once values load); asc/desc; folders-first toggle in Settings (default on). **Name** order matches Windows Explorer (`StrCmpLogicalW`: digit runs as numbers; more leading zeros sort first).
