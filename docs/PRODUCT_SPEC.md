...
- **Quick Launch (D63)** — toolbar icons for user-picked programs (name, path, optional args, shell or custom icon). Settings → Quick Launch adds / edits / removes (no toolbar +; strip hidden when empty). Click launches; right-click Open file location / Remove; drop an `.exe` / shortcut on the strip to add when it is visible. Cap 24.
...
| Quick Launch | Toolbar apps: add / name / path / arguments / icon / order (D63)                   |
