...
1. **Tab bar** — default: each tab is only as wide as its title (capped ~220px, min ~90px). Settings → Appearance **Equal-width tabs** restores “all tabs match the widest label” (icon-only custom tabs stay tight and do not stretch). Overflow uses ◀ ▶ scroll (active tab stays in view; mouse wheel scrolls the strip); drag reorder; optional colored Lucide/Phosphor/Tabler icon + title, or a custom image (label on/off, size); icon-only tabs hug the image so they work as categorizer drop bins; double-click title to rename; right-click for Duplicate / Rename / Set icon / **Set drop shortcut…** / **Move to new window** / Close; a floating window also offers **Merge into main window**. Closing that window asks whether to merge or close its tabs. **Reopen closed window** restores the last closed float (drag-off is not available yet). `×` close; `+` new tab; drag tab onto a view pane to assign/move it (**Ctrl+drag** duplicates so both panes can show the same path). **Drop shortcut** (optional per tab): press the chord to move/copy the focused pane’s selection into that tab’s current folder (mode Auto / Always move / Always copy); shown in the tab tooltip when set.
