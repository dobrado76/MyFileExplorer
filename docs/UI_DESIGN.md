...
1. **Tab bar** — equal-width tabs that shrink toward a min (~90px) then overflow with ◀ ▶ scroll (active tab stays in view; mouse wheel scrolls the strip); drag reorder; optional colored Lucide icon + title; double-click title to rename; right-click for Duplicate / Rename / Set icon / Close; `×` close; `+` new tab; drag tab onto a view pane to assign/move it (**Ctrl+drag** duplicates so both panes can show the same path)
