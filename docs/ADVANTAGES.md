...
**App:** MyFileExplorer · **Version:** 0.17.0
...
| **ACDSee-inspired slideshow** | Gated fullscreen slideshow + categorizer map (keyboard folder/delete buffer, commit on stop) and image-list cache — Explorer has no equivalent. |
...
| **In-app Power Rename** | Search/replace (literal, DOS `*`/`?`, or regex) plus Advanced options (numbering, case, dates, …) across a selection with live preview — no PowerToys shell extension required. Guide: [POWER_RENAME.md](POWER_RENAME.md). |
