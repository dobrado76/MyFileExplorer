...
- Settings → **Slideshow**: delay (`0` = as fast as decode/display allows; no upper cap), order, ascending, loop, **Include subfolders** (`slideshow.recursive`, default on — off = selected folder only), **draw caption** (Caption ADS poster), invalid-images folder
