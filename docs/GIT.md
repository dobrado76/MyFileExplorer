...
**Commit row context menu** (GE-inspired): Copy submenu; Merge into current branch; Rebase current branch on (non-interactive); Reset soft/mixed/hard (confirms; hard is strong); Create branch/tag here; Checkout (detached); Revert; Cherry-pick; Navigate (parents/HEAD in loaded list); View (refresh / terminal). Conflicts surface via notify + status refresh — no conflict editor.
