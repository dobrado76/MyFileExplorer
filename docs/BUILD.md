...
| Tag `v*` (e.g. `v0.17.0`) | Check + build **three** Windows executables: `MyFileExplorer.exe` + `MfeShellLauncher.exe` (bundled in the NSIS installer) and `MfeVirtualFolderService.exe` (separate zip) → attach to a **GitHub Release** |
...
```bash
# package.json version should match the tag (e.g. 0.17.0)
git tag v0.17.0
