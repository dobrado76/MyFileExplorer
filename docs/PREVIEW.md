...
- **No ffmpeg remux for Preview playback** (locks the file for rename/move/delete). Default: `.mkv` / `.ts` / `.m2ts` / `.wmv` / `.avi` / … → **Open with default app** (optional still/strip). Settings → Preview: **Autoplay media in preview** (`previewVideoAutoplay`, default off); **Rich player (mpv)** (`previewRichPlayerMpv`, default **off**, D33) — when on, embeds a bundled/PATH `mpv` helper for hostile containers/codecs in-pane (`npm run tools:fetch-mpv`); Open with default app stays as fallback.
...
- On decode error for otherwise playable types: short message + open-with-default-app button (and poster if available); with Rich player on, attempt mpv before that fallback
