...
- **Async tags:** `preview:get` returns `mediaUrl` / optional poster without waiting on `music-metadata` (duration scans can read the whole file). Tag fields arrive via `preview:getMediaMeta` in parallel while the player buffers (`mediaMetaPending`)
- Playback depends on Chromium’s codecs. **H.264 + AAC** in MP4/M4V/MOV usually works. **HEVC (H.265)** uses the **OS decoder** (`PlatformHEVCDecoderSupport`) — Windows: HEVC Video Extensions. **AV1 / VP9** when Chromium supports them. **AC-3 / DTS** → Open with default app.
- **No ffmpeg remux for Preview playback** (locks the file for rename/move/delete). `.mkv` / `.ts` / `.m2ts` / `.wmv` / `.avi` / … → **Open with default app** (optional still/strip). Settings → Preview: **Autoplay media in preview** (`previewVideoAutoplay`, default off)
