...
- **Async tags:** `preview:get` still returns `mediaUrl` without waiting on `music-metadata` (duration scans can read the whole file). The renderer starts `preview:getMediaMeta` in parallel and **does not paint the player** until tags are merged (or the parse fails). That keeps the File/VIDEO layout stable — no late strip insertion.
