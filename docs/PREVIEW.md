...
- Containers Chromium can’t demux (`.mkv`, `.wmv`, `.flv`, …): still (`posterUrl`) then remux/transcode to MP4 under `userData/video-remux/` (session temp; `preview:ensurePlayable`) when practical. Settings → Preview: **Autoplay media in preview** (`previewVideoAutoplay`, default off)
